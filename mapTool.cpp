...
HRESULT mapTool::init()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		sampleImage[i]->init("images/맵툴던전.bmp", 600 * 2, 300 * 2, 12, 6, true, RGB(255, 0, 255));
	}
...
OBJECT mapTool::dungeonObjSelect(int frameX, int frameY)
{
	for (int i = 5; i < 10; i++)
	{
		for (int j = 0; j < 7; j++)
		{
			...
			if (i == 8 && j == 0) return OBJ_BOX;

			if (i >= 5 && i<=7 && j >= 6) return OBJ_TENT;
		}
	}
